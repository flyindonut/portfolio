...
<template>
  <div class="flex h-dvh overflow-y-scroll">
    ...
    >
      ...
      <!-- No Services Message -->
      <p v-if="!errorMessage && services.length === 0" class="text-[var(--text-2-color)] mt-6">No services available.</p>
      ...
      >
        ...
        >
          ...
          <span class="mt-2 text-[var(--text-color)] text-sm">{{ service.translations.en.name }}</span>
  ...
  <div v-if="showModal" class="fixed inset-0 flex items-center justify-center bg-opacity-50 backdrop-blur-md z-50">
    <div class="bg-[var(--bg-color)] p-6 rounded-xl shadow-2xl border border-gray-700 text-[var(--text-color)] w-full max-w-md mx-4">
      ...
      <form @submit.prevent="handleCreateService" class="space-y-4">
        ...
        <!-- English Name Input -->
        <div>
          <label class="block text-sm font-medium text-[var(--text-3-color)]">Name (EN)</label>
          <input v-model="newService.translations.en.name" type="text" class="w-full bg-[var(--input-color)] text-[var(--text-color)] border border-gray-600 rounded-lg p-2 focus:ring focus:ring-gray-500" required />
        </div>

        <!-- French Name Input -->
        <div>
          <label class="block text-sm font-medium text-[var(--text-3-color)]">Name (FR)</label>
          <input v-model="newService.translations.fr.name" type="text" class="w-full bg-[var(--input-color)] text-[var(--text-color)] border border-gray-600 rounded-lg p-2 focus:ring focus:ring-gray-500" required />
        </div>
